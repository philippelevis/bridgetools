...
public class SideNinja implements Bridge, ClientTickEvents.StartTick {
    public boolean alreadysetup = false;
    @Override
    public void onStartTick(MinecraftClient client) {
        if(BridgetoolsClient.Modules.get("SideNinja")) {
    ...
    private void setupBridge(PlayerEntity player){
        if(player != null) {
            ...
            float yaw = (float) (Math.round(yawb4 / 90) * 90)+45;
            float pitch = 80;
            player.setYaw(yaw);
